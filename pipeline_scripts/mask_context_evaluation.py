...
class Context_Iterator:
    ...
    def getContourData(self,ContourFiles,RingFile, label_classes,label_classNames):
        DeeplabVIARings = {}
        DeeplabVIAPoints = {}
        DeeplabVIAFrames = {}
        ...
        for file in ContourFiles:
            fileID, epoch = self.fileID_and_Epoch(file)
            #if "frame" not in file:
            label_class = label_classNames[index]
            if "ring" in label_class:
                ...
                class_name = label_classNames[index] #J.getClassKey(label_class)
        ...
        if RingFile != "":
            VIAFname = RingFile
            fileID, epoch = self.fileID_and_Epoch(RingFile)
            J = utils.VIARingJSON(VIAFname)
            if fileID not in DeeplabVIARings.keys():
                DeeplabVIARings[fileID] = {}
            #if fileID not in DeeplabVIAFrames.keys():
